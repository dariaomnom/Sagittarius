...
void calculateEquatorialCoordinates(double x, double y, double z, double* result) {
	// ¬ычисл€ем пр€мое восхождение (alpha) в радианах
	double alpha = atan2(y, x); // atan2 учитывает знаки x и y

	// ¬ычисл€ем склонение (delta) в радианах
	double r = sqrt(x * x + y * y + z * z); // рассто€ние от начала координат
	double delta = asin(z / r); // склонение = arcsin(z / r)

	// ѕреобразуем результаты из радиан в секунды дуги
	result[0] = alpha * RAD_TO_ARCSECONDS; // ѕр€мое восхождение в секундах дуги
	result[1] = delta * RAD_TO_ARCSECONDS; // —клонение в секундах дуги
}

void calculateCartesianCoordinates(double alpha_arcseconds, double delta_arcseconds, double r, double* result) {
	// ѕереводим пр€мое восхождение и склонение из аркасекунд в радианы
	double alpha = alpha_arcseconds / RAD_TO_ARCSECONDS;
	double delta = delta_arcseconds / RAD_TO_ARCSECONDS;

	// ¬ычисл€ем декартовы координаты
